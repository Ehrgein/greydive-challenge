  ...
  return (
    <div className='testingcontent'>
        <div className='testdata'>
            ...
            <h1 className='clientname'>Test: Test de usabilidad en el {db[testid].id}</h1>
            <h1 className='testername'> Testeador 1 </h1>
            <video className='video' controls="controls autoplay"
             src={db[testid].linkVideo} type="video/mp4"/>
            <h1 className='transcription'>Transcripción</h1>

            <div className='transcriptiontxt'>
                        {cleantranscript}
            </div>
            <div className='transcription'>
                ...
                {db[testid].preguntas.map((item, index) => (
                    ...
                    <div key={index} className='taskcontainer'>
                        <strong className='infotarea' key={index}>
                            Tarea: {index +1}
                            <br/>
                        {item.texto.replaceAll("\\n", "\n\n")}</strong>

                        {/[0-9]/.test(item.respuesta) ? <p className='respuesta'> Respuesta: {item.respuesta}</p> : null}
                        <p className='duraciontarea'> Duracion de la tarea: {item.tiempo}</p>
                    </div>
                ))}
            </div>
        </div>
